...
EVAL_GUIDELINES = (
    "Did the agent use the right HITL pattern (confirmation for restarts, "
    "user input for tickets, external execution for diagnostics)? "
    "Extra diagnostic steps before a restart are acceptable."
)
...
def run_evals(category: str | None = None) -> None:
    """Run evaluation suite for Guard."""
    ...
    print(f"Running {len(tests)} Guard evals...")
    ...
    for idx, tc in enumerate(tests, 1):
        ...
        response = guard.run(tc.question)

        # Accuracy: LLM judge compares output vs expected
        AccuracyEval(
            db=eval_db,
            name=f"Guard Accuracy - {tc.category}",
        ...
        if tc.expected_tools:
            ReliabilityEval(
                db=eval_db,
                name=f"Guard Reliability - {tc.category}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Guard evaluations")
