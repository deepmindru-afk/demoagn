"""
LLM-based grader for evaluating MCP agent responses.

Uses a small, fast model to evaluate if the agent's response correctly
answers questions using MCP tool lookups.
"""
...
You are an evaluation grader for an MCP-based documentation agent. Your job is to determine if the agent's
response correctly answers the user's question about the Agno framework using MCP tools.
...
- Tool usage: Did the agent appropriately use its MCP tools?
