...
EVAL_GUIDELINES = (
    "Did the agent use the right governance pattern (auto-execute for lookups and docs, "
    "confirmation for refunds, user feedback for missing info)? "
    "Extra lookup steps before a refund are acceptable."
)
...
def run_evals(category: str | None = None) -> None:
    """Run evaluation suite for Support."""
    ...
    print(f"Running {len(tests)} Support evals...")
    ...
    for idx, tc in enumerate(tests, 1):
        ...
        response = support.run(tc.question)

        # Accuracy: LLM judge compares output vs expected
        AccuracyEval(
            db=eval_db,
            name=f"Support Accuracy - {tc.category}",
        ...
        if tc.expected_tools:
            ReliabilityEval(
                db=eval_db,
                name=f"Support Reliability - {tc.category}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Support evaluations")
